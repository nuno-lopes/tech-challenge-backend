...
describe('plugin', () => describe('actor', () => {
    ...
    before(async ({ context }: Flags) => {
        context.stub = {
            ...
            lib_find_favorite_genre: sandbox.stub(lib, 'findFavoriteGenre'),
        }
    ...
    beforeEach(({ context }: Flags) => {
        ...
        context.stub.lib_find_favorite_genre.rejects(new Error('test: provide a mock for the result'))
    })
    ...
    describe('GET /actors/:id/favoriteGenre', () => {
        const paramId = 123
        const [method, url] = ['GET', `/actors/${paramId}/favoriteGenre`]
        ...
        it('returns actor favorite genre', async ({ context }: Flags) => {
            ...
            const opts: Hapi.ServerInjectOptions = { method, url }
            const anyResult = { 'any': 'result' }
            context.stub.lib_find_favorite_genre.resolves(anyResult)

            const response = await context.server.inject(opts)
            expect(response.statusCode).equals(200)

            sinon.assert.calledOnceWithExactly(context.stub.lib_find_favorite_genre, paramId)
            expect(response.result).equals(anyResult)
        })
